...
public class CyclicNoCovariateDifferentVarCrossingDenseGaussTwoModeNetworkModel
	...
	public CyclicNoCovariateDifferentVarCrossingDenseGaussTwoModeNetworkModel() {
	}
					...
					for (int j = 0; j < graph.getNumOfColumns(); j++)
						for (int l = 0; l < numOfColumnGroups; l++) {
							...
							if (logProb < 0) {
								...
								zS[index][k] += (z[i][k] + w[j][l]) * logProb;
							}
						}
		} catch (Exception e) {
			e.printStackTrace();
					...
					for (int i = 0; i < graph.getNumOfRows(); i++)
						for (int k = 0; k < numOfRowGroups; k++) {
							...
							if (logProb < 0) {
								...
								wS[index][l] += (z[i][k] + w[j][l]) * logProb;
							}
						}
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected void updateModelParamters() {
		try {
			...
			System.out
					.println("ERROR when call graph access methods in estimateMeansVariances()!!!");
			e.printStackTrace();
			System.exit(-1);
		}
	}
