...
public class BICAdditiveSparseBinaryTwoModeNetworkSimulator extends
		AdditiveSparseBinaryTwoModeNetworkSimulator {
		...
		try {
			...
			for (int runningRowGroups = 1; runningRowGroups <= 2 * numRowGroups; runningRowGroups++) {
				for (int runningColGroups = 1; runningColGroups <= 2 * numColumnGroups; runningColGroups++) {
					...
					emEngine.setDataFiles(TwoModeEMEngine.SPARSE_DATA_FORMAT,
