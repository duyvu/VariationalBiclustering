...
public class CrossedDenseGaussTwoModeNetworkSimulator {

	protected static BinaryCovariateCrossingDenseGaussTwoModeNetworkSampler sampler = new BinaryCovariateCrossingDenseGaussTwoModeNetworkSampler();
	...
	protected static void runClustering() {
		...
		emEngine.setDataFiles(TwoModeEMEngine.MATRIX_DATA_FORMAT, delim,
		...
		double[] MSEs = emEngine.computeRowMSEs(sampler.getRowBeta());
		...
		try {
			PrintStream MSEsOUT = new PrintStream(new File(outputDir + "/MSEs-"
					+ seed0 + ".txt"));
			for (int k = 0; k < MSEs.length; k++)
				MSEsOUT.print(MSEs[k] + "\t");
			MSEsOUT.close();
